...
## 转换方块和物品ID
同一个方块和物品的数字ID在客户端和服务器上很可能是不同的。 Inner Core会自动同步ID，所以同一个物品在不同设备上会有不同的ID。 但是，如果它们是以数字的形式传输的，那将会是不同的，必须手动转换
...
```js
//从服务器传到客户端的数据包，包含了关于物品使用的信息（例如，我们只传递id和数据）
Network.addClientPacket("testMod.useItemInfo", function() {
    //显示信息：首先是server发来的id，然后是这个物品在客户端上的id，为了确认这是客户端上的同一个物品，我们通过clientid来获取它的名字。
